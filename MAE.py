...
class VQMAE(nn.Module):
    ...
    def forward(self, x, temb=None):
        ...
        # print("Regerage decoded image : {}".format(decoded_images.shape)) # torch.Size([1, 3, 256, 256])
...
class MAE(nn.Module):
    ...
    def forward(self, x, temb=None):
        x_masked, mask, ids_restore = self.encode_forward(x)
        quant_conv_encoded_images  = self.encode_reshape_forward(x_masked, ids_restore)
        decoded_images = self.decoder(quant_conv_encoded_images)

        # print("Regerage decoded image : {}".format(decoded_images.shape)) # torch.Size([1, 3, 256, 256])
        return decoded_images, quant_conv_encoded_images
    @staticmethod
    def adopt_weight(disc_factor, i, threshold, value=0.):
        if i < threshold:
            disc_factor = value
